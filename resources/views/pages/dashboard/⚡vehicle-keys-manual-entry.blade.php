...
new class extends Component
{
    ...
    {
        $this->keys = VehicleKey::all();
        $this->loadDraftEntries();
    }

    public function loadDraftEntries()
    {
        $this->draftEntries = DraftKeyBorrowingsEntry::with('vehicleKey')
            ->get()
            ->map(function ($entry) {
    ...
    public function addEntry()
    {
        ...
        Flux::modal('vehicle-keys-manual-entry')->close();
    }
    ...
    public function submitAll()
    {
        ...
        foreach ($drafts as $draft) {
            KeyBorrowing::create([
                'vehicle_key_id' => $draft->vehicle_key_id,
    ...
    public function verifyAndSubmit()
    {
        ...
        Flux::modal('vehicle-keys-submit-verification')->close();
    }
};
?>
...
<x-pages::dashboard.keys>
    <div class="flex justify-between mb-8">
        <div>
            <flux:heading>Manual Entry - Vehicle Keys</flux:heading>
    ...
    <div class="grid grid-cols-4 gap-2 items-end">
        ...
        <div class="col-span-1">
            <flux:modal.trigger name="vehicle-keys-manual-entry">
    ...
    <div class="mt-4">
        <flux:modal.trigger name="vehicle-keys-submit-verification">
        ...
        <flux:table>
            ...
            <flux:table.rows>
                @forelse($completedEntries as $entry)
                    <flux:table.row>
                        <flux:table.cell>{{ $entry->vehicleKey->vehicle_number }}</flux:table.cell>
                        <flux:table.cell>{{ $entry->borrower_name }}</flux:table.cell>
                        <flux:table.cell>{{ $entry->borrowed_at }}</flux:table.cell>
                        <flux:table.cell>{{ $entry->returned_name }}</flux:table.cell>
                        <flux:table.cell>{{ $entry->returned_at }}</flux:table.cell>
    ...
    <flux:modal name="vehicle-keys-submit-verification" class="w-[1000px]!">
        <div class="space-y-6">
            <div>
    ...
    <flux:modal name="vehicle-keys-manual-entry" class="md:w-200" :dismissible="false">
